...
class TransactionsController extends Controller
{
    ...
    {
        $transactions = DB::select('select T.id, T.trade_date, U.first_name from transactions T left join users U on U.id = T.client_id where T.deleted_at IS NULL ORDER BY T.created_at DESC');
    ...
    {
        $clients_rec = DB::select('select U.first_name, U.last_name, U.id from users U left join role_user UR on U.id = UR.role_id left join roles R on R.id = UR.role_id WHERE R.id NOT IN ("1","4") ORDER BY U.first_name ASC');
        $clients = [];
