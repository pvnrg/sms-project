    ...
    <div class="col-md-6">
        ...
        <div class="form-group {{ $errors->has('name') ? 'has-error' : ''}}">
            <label for="title" class="">
                ...
                {!! Form::text('name', isset($fee_schedules->name) ? $fee_schedules->name : '', ['class' => 'form-control']) !!}
                {!! $errors->first('name', '<p class="help-block">:message</p>') !!}

        </div>
         <div class="form-group {{ $errors->has('fees') ? 'has-error' : ''}}">
            <label for="title" class="">
                <span class="field_compulsory">*</span>@lang('class.label.fees')
            </label>
                {!! Form::text('fees', isset($fee_schedules->fees) ? $fee_schedules->fees : '', ['class' => 'form-control']) !!}
